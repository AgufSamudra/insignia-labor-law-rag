      ...
      <section className="workspace">
        ...
        {activeTab === 'asking' ? <section className="panel asking-panel">
          ...
          {queryResponse ? <div className="query-result" aria-live="polite">
            ...
            {queryResponse.retrieval.rewritten_query ? <div className="rewritten-query"><span>SYSTEM REWRITE</span><p>{queryResponse.retrieval.rewritten_query}</p></div> : null}
            <div className="answer-copy">{queryResponse.answer}</div>
            {queryResponse.sources.length > 0 ? <div className="source-section"><p className="list-label">SOURCES · CLICK TO OPEN</p><ol className="source-list">{queryResponse.sources.map((source) => <li className="source-item" key={`${source.id}-${source.chunk_id}`}><details><summary className="source-summary"><strong>{source.document}</strong><span>Halaman {source.page}{source.page_end && source.page_end !== source.page ? `–${source.page_end}` : ''}{source.article ? ` · ${source.article}` : ''}{source.paragraph ? ` · ${source.paragraph}` : ''}</span>{typeof source.rerank_score === 'number' ? <span className="source-score">Rerank score · {source.rerank_score.toFixed(6)}</span> : null}</summary><p className="source-content">{source.text ?? 'Isi sumber tidak tersedia.'}</p></details></li>)}</ol></div> : null}
          </div> : null}
